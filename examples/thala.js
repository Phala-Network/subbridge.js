...
async function main() {
    ...
    console.log(`\n============ Receiving count =============\n${JSON.stringify(await Indexer.paraReceiveCount('thala', '0x7804e66ec9eea3d8daf6273ffbe0a8af25a8879cf43f14d0ebbb30941f578242'), null, 2)}`);
    // Para: Query all receving history by recipient
    console.log(`\n============ All receiving history =============\n${JSON.stringify(await Indexer.paraReceiveHistory('thala', '0x7804e66ec9eea3d8daf6273ffbe0a8af25a8879cf43f14d0ebbb30941f578242'), null, 2)}`);
    // Para: Query limitted recving history by recipient
    console.log(`\n============ Latest 3 receiving history =============\n${JSON.stringify(await Indexer.paraLimittedReceiveHistory('thala', '0x7804e66ec9eea3d8daf6273ffbe0a8af25a8879cf43f14d0ebbb30941f578242', 3), null, 2)}`);
    // Para: Range query receving history
    console.log(`\n============ Range receving history [0, 100] =============\n${JSON.stringify(await Indexer.paraRangeReceiveHistory('thala', '0x7804e66ec9eea3d8daf6273ffbe0a8af25a8879cf43f14d0ebbb30941f578242', 0, 100), null, 2)}`);
    // Para: Query receiving confirmation of a specific incoming transaction
    console.log(`\n============ Confirmation of transaction{originChain: thala, destChain: thala, depositNonce: 9} =============\n${JSON.stringify(await Indexer.paraChainbridgeReceiveConfirm('thala', Indexer.chainbridgeChainid('thala'), 9), null, 2)}`);
}
