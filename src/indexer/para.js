...
export async function paraSendCount(network, account) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
    let data
    try {
...
export async function paraSendHistory(network, sender) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
...
export async function paraLimittedSendHistory(network, sender, limit) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
...
export async function paraRangeSendHistory(network, sender, from, to) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
    ...
    try {
      data = await client.request(gql`
            {
                xTransferSents (orderBy: CREATED_AT_DESC, filter: {sender: {equalTo: \"${sender.toLowerCase()}\"}, index: {greaterThanOrEqualTo: ${Number(
        from
...
export async function paraReceiveCount(network, recipient) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
...
export async function paraReceiveHistory(network, recipient) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
...
export async function paraLimittedReceiveHistory(network, recipient, limit) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
...
export async function paraRangeReceiveHistory(network, recipient, from, to) {
  if (network.toLowerCase() === 'thala') {
    let client = new GraphQLClient(GraphEndpoint[network.toLowerCase()], {
      timeout: 300000,
    })
    ...
    try {
      data = await client.request(gql`
            {
                xTransferDepositeds (orderBy: CREATED_AT_DESC, filter: {isLocal: {equalTo: true}, account: {equalTo: \"${recipient.toLowerCase()}\"}, index: {greaterThanOrEqualTo: ${Number(
        from
      )}, lessThanOrEqualTo:${Number(to)}}}) {
                    nodes {
                        id
...
export async function paraChainbridgeReceiveConfirm(
  network,
  originChainId,
  depositNonce
) {
